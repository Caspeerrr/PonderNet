...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, data_dir='./', test_transform=None, batch_size=256, dataset=CIFAR10):
        super().__init__()
    ...
    def setup(self, stage=None, validation_size=5000):
        ...
        if stage in [None, 'fit', 'validate']:
            mnist_train = self.dataset(self.data_dir, train=True,
                                transform=(self.train_transform))
            mnist_val = self.dataset(self.data_dir, train=True,
        ...
        if stage == 'test' or stage is None:
            if self.test_transform is None or isinstance(self.test_transform, transforms.Compose):
                self.mnist_test = self.dataset(self.data_dir,
                                        train=False,
    ...
    def test_dataloader(self):
        ...
        if isinstance(self.mnist_test, self.dataset):
...
def get_transforms():
    ...
    for s in range(1, 6):
        ...
        names.append([s, 'contrast transform'])
        ...
        names.append([s, 'rotation transform'])
